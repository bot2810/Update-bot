...
# ✅ FLASK API CONFIG
app = Flask(__name__)
API_SECRET_KEY = os.getenv('API_SECRET_KEY', 'your_secret_api_key_here_change_this')  # Change this to a secure key
API_ENDPOINTS = {
...
def add_balance_to_user(user_id, amount, source="Unknown", admin_id=None):
    """Centralized function to add balance to user - works for both API and commands"""
    try:
        # Thread-safe balance update
        with data_lock:
            old_balance = user_balances.get(user_id, 0)
            
            # Prevent negative balance
            if amount < 0 and old_balance + amount < 0:
                if admin_id:
                    bot.send_message(admin_id, f"❌ **Insufficient balance for deduction**\n\n📊 **User Balance:** ₹{old_balance:.2f}\n💸 **Requested Deduction:** ₹{abs(amount):.2f}\n❌ **Result:** Would be negative (₹{old_balance + amount:.2f})", parse_mode="Markdown")
                return False
            
            user_balances[user_id] = max(0, old_balance + amount)
            new_balance = user_balances[user_id]

        # Save data immediately
        if save_data():
            operation = "added" if amount >= 0 else "deducted"
            logger.info(f"💰 Balance updated via {source} - User: {user_id}, Amount: {amount:+.2f}, Operation: {operation}")

            # Send notification to user
            try:
                if amount >= 0:
                    notification_message = f"💰 **Balance Added!**\n\n"
                    notification_message += f"✅ ₹{amount:.2f} has been added to your account!\n\n"
                else:
                    notification_message = f"💸 **Balance Deducted!**\n\n"
                    notification_message += f"⚠️ ₹{abs(amount):.2f} has been deducted from your account!\n\n"
                ...
                notification_message += f"🌐 **Source:** {source}\n"
                notification_message += f"⏰ **Time:** {get_local_time()}"

                bot.send_message(user_id, notification_message, parse_mode="Markdown")
                
                # Send notification to admin if this was API call or confirm if command
                if admin_id:
                    # This is from admin command - send confirmation
                    admin_confirm = f"✅ **Balance Update Successful!**\n\n"
                    admin_confirm += f"👤 **User ID:** {user_id}\n"
                    admin_confirm += f"💰 **Amount {operation.title()}:** ₹{abs(amount):.2f}\n"
                    admin_confirm += f"📊 **New Balance:** ₹{new_balance:.2f}\n"
                    admin_confirm += f"📤 **User Notified:** Yes\n"
                    admin_confirm += f"⏰ **Time:** {get_local_time()}"
                    
                    bot.send_message(admin_id, admin_confirm, parse_mode="Markdown")
                else:
                    # This is from API - notify admin about the automatic addition
                    admin_notification = f"🤖 **Automatic Balance Addition**\n\n"
                    admin_notification += f"👤 **User ID:** {user_id}\n"
                    admin_notification += f"💰 **Amount:** ₹{amount:.2f}\n"
                    admin_notification += f"🌐 **Source:** {source}\n"
                    admin_notification += f"📊 **New Balance:** ₹{new_balance:.2f}\n"
                    admin_notification += f"⏰ **Time:** {get_local_time()}\n\n"
                    admin_notification += f"✅ **Status:** Successfully processed automatically"
                ...
                # Even if notification fails, confirm success to admin if command
                if admin_id:
                    admin_confirm = f"✅ **Balance Update Successful!**\n\n"
                    admin_confirm += f"👤 **User ID:** {user_id}\n"
                    admin_confirm += f"💰 **Amount {operation.title()}:** ₹{abs(amount):.2f}\n"
                    admin_confirm += f"📊 **New Balance:** ₹{new_balance:.2f}\n"
                    admin_confirm += f"⚠️ **User Notification:** Failed - {str(e)}\n"
                    admin_confirm += f"⏰ **Time:** {get_local_time()}"
                    
                    bot.send_message(admin_id, admin_confirm, parse_mode="Markdown")
                
                logger.warning(f"Failed to send notifications for balance addition: {e}")
                return True  # Balance was still added successfully
        else:
            if admin_id:
                bot.send_message(admin_id, "❌ **Error:** Failed to save balance data", parse_mode="Markdown")
            return False

    except Exception as e:
        logger.error(f"Error in add_balance_to_user: {e}")
        return False
...
def auto_add_balance_for_task(user_id, task_text, task_section, task_index):
    ...
    try:
        ...
        if reward >= 0.1:
            success = add_balance_to_user(user_id, reward, f"Task Auto-Reward ({task_section})")
            
            # Mark task as completed for limited sections
            if success and task_section in ['app_downloads', 'promotional', 'watch_ads']:
    ...
    if user_id == ADMIN_ID:
        ...
        if text.startswith("/addbalance"):
            try:
                parts = text.split()
                if len(parts) != 3:
                    bot.send_message(ADMIN_ID, "⚠️ **Usage:** `/addbalance user_id amount`\n\n💡 **Examples:**\n• `/addbalance 123456789 25.50` - Add ₹25.50\n• `/addbalance 123456789 -10` - Deduct ₹10\n• `/addbalance 123456789 0.10` - Add minimum ₹0.10\n\n🔢 **Minimum Amount:** ₹0.10", parse_mode="Markdown")
                    return
                ...
                try:
                    ...
                    # Check minimum amount validation (0.10 minimum for addition)
                    if amount > 0 and amount < 0.10:
                        bot.send_message(ADMIN_ID, "❌ **Minimum amount is ₹0.10**\n\n💡 Use `/addbalance user_id 0.10` for minimum addition", parse_mode="Markdown")
                        return
                        
                except ValueError:
                    bot.send_message(ADMIN_ID, "❌ **Invalid amount format**\n\n💡 **Examples:**\n• `25.50` for ₹25.50\n• `-10` for deduct ₹10\n• `0.10` for minimum ₹0.10", parse_mode="Markdown")
                    return

                # FIXED: Actually process the balance addition
                with data_lock:
                ...
                if save_data():
                    operation = "added" if amount >= 0 else "deducted"
                    logger.info(f"💰 Balance updated via Admin Command - User: {target_id}, Amount: {amount:+.2f}, Operation: {operation}")

                    # Send notification to user
                    try:
                        if amount >= 0:
                            user_notification = f"💰 **Balance Added by Admin!**\n\n"
                            user_notification += f"✅ ₹{amount:.2f} has been added to your account!\n\n"
                        else:
                            user_notification = f"💸 **Balance Deducted by Admin!**\n\n"
                            user_notification += f"⚠️ ₹{abs(amount):.2f} has been deducted from your account!\n\n"

                        user_notification += f"📊 **Balance Update:**\n"
                        user_notification += f"   • Previous: ₹{old_balance:.2f}\n"
                        user_notification += f"   • {operation.title()}: ₹{abs(amount):.2f}\n"
                        user_notification += f"   • Current: ₹{new_balance:.2f}\n\n"
                        user_notification += f"👨‍💼 **Source:** Admin Command\n"
                        user_notification += f"⏰ **Time:** {get_local_time()}"
                    ...
                    admin_confirm = f"✅ **Balance Command Executed Successfully!**\n\n"
                    admin_confirm += f"👤 **User ID:** {target_id}\n"
                    admin_confirm += f"💰 **Amount {operation.title()}:** ₹{abs(amount):.2f}\n"
                    admin_confirm += f"📊 **New Balance:** ₹{new_balance:.2f}\n"
                    admin_confirm += f"📤 **User Notified:** Yes\n"
                    admin_confirm += f"💾 **Data Saved:** Yes\n"
                    admin_confirm += f"⏰ **Time:** {get_local_time()}"
                ...
                bot.send_message(ADMIN_ID, f"❌ **Error processing /addbalance command:**\n\n{str(e)}", parse_mode="Markdown")
                logger.error(f"Admin addbalance command error: {e}")
...
# ✅ FLASK API ROUTES FOR EXTERNAL WEBSITES
@app.route(API_ENDPOINTS['add_balance'], methods=['POST'])
def api_add_balance():
    """API endpoint for external websites to add balance automatically"""
    try:
        ...
        source = data.get('source', 'External API')  # Optional: track source
        ...
        try:
            ...
            return jsonify({
                'success': False,
                'error': 'Invalid user_id or amount format',
                'code': 400
            }), 400
        ...
        # Use centralized balance addition function
        success = add_balance_to_user(user_id, amount, source)
        
        if success:
            new_balance = user_balances.get(user_id, 0)
            return jsonify({
                'success': True,
                'message': 'Balance added successfully via API',
                'data': {
                    'user_id': user_id,
                    'amount_added': amount,
        ...
        logger.error(f"API add balance error: {e}")
        return jsonify({
            'success': False,
...
def api_check_balance():
    ...
    try:
        ...
        logger.error(f"API check balance error: {e}")
...
def api_user_info():
    ...
    try:
        ...
        referral_count = sum(1 for ref_id in referral_data.values() if ref_id == user_id)
        completed_tasks_count = len(completed_tasks.get(user_id, set()))
...
def api_status():
    """API endpoint to check if the bot API is working"""
    return jsonify({
        'success': True,
...
# ✅ ENHANCED BOT RUN FUNCTION WITH FLASK SERVER
...
def run_bot():
    ...
    try:
        ...
        logger.info(f"🔑 API Secret Key: {API_SECRET_KEY}")
        logger.info("📋 Available API Endpoints:")
        for endpoint_name, endpoint_path in API_ENDPOINTS.items():
            logger.info(f"   • {endpoint_name}: POST {endpoint_path}")
    ...
    while restart_count < max_restarts:
        try:
            ...
            logger.info("🔒 Emergency freeze code: ACTIVE")
            logger.info("🔓 Emergency unlock code: ACTIVE")
            logger.info("🌐 FLASK API SERVER: ACTIVE ON PORT 5000")
            logger.info("🤖 AUTOMATIC BALANCE ADDITION: ENABLED VIA API")
            logger.info("🚀 Bot ready with ALL ENHANCED TRACKING & SECURITY FEATURES + API!")
